...
cdef extern from "topo_core.h":
    ...
    void horval(int n, double *z, double delta, int *h, double *hcos,double*hcos_front,double*hcos_back);
    void hor2d(int n, int m, double *z, double delta, bint forward, double *hcos,double*hcos_front,double*hcos_back);
...
def c_hor1d(np.ndarray[double, mode="c", ndim=1] z,
           ...
           np.ndarray[double, mode="c", ndim=1] hcos_front,
           np.ndarray[double, mode="c", ndim=1] hcos_back,
):
    """
    ...
    horval(n, &z_arr[0], spacing, &h[0], &hcos[0],&hcos_front[0],&hcos_back[0])
...
def c_hor2d(np.ndarray[double, mode="c", ndim=2] z,
           ...
           np.ndarray[double, mode="c", ndim=2] hcos_front,
           np.ndarray[double, mode="c", ndim=2] hcos_back):
    """
    Call the function hor1f in hor1f.c
    ...
    hor2d(nrows, ncols, &z_arr[0,0], cspacing, fwd, &hcos[0,0],&hcos_front[0,0],&hcos_back[0,0])
