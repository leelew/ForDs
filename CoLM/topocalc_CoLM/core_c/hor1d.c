...
void hor2d(
    ...
    double *hcos_front, /* cosines of angles to horizon front*/
    double *hcos_back) /* cosines of angles to horizon back*/
{
    int i, j; /* loop index */
    ...
    */
    ...
    double *front;
    front=(double *)calloc(ncols,sizeof(double));

    double *back;
    back=(double *)calloc(ncols,sizeof(double));
    ...
    for (i = 0; i < nrows; i++)
    {
    	 ...
    	 */
        horval(ncols, zbuf, delta, hbuf, obuf,front,back);

        for (j = 0; j < ncols; j++)
        {
            hcos[i * ncols + j] = obuf[j];
            hcos_front[i*ncols+j]=front[j];
            hcos_back[i*ncols+j]=back[j];
        }
    }
}
...
void horval(
    ...
    double *hcos, /* cosines of angles to horizon */
    double *hcos_front, /* cosines of angles to horizon */
    double *hcos_back) /* cosines of angles to horizon */
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        else
        {
            ...
            hcos_front[i]=diff/(double)hypot(diff, d * delta-0.5*delta);
            hcos_back[i]=diff/(double)hypot(diff, d * delta+0.5*delta);
