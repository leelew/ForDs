...
def viewf(dem, spacing, slope, aspect, nangles=36, sin_slope=None):
    """
    ...
    # record the sin(terrain elevation angle)
    tea, tea_front, tea_back=[], [], []

    # perform the integral
    for angle in angles:

        # caculate cos(horizon angles)
        hcos, hcos_front, hcos_back = horizon(angle, dem, spacing)

        # cos(horizon angles) = sin(terrain elevation angle)
        tea.append(hcos)
        tea_front.append(hcos_front)
        tea_back.append(hcos_back)
    ...
    # caculate terrain configuration factor
    tcf = (1 + cos_slope)/2 - svf
    
    # make sin(terrain elevation angle) array
    tea = np.stack(tea, axis=-1)
    tea_front = np.stack(tea_front, axis=-1)
    tea_back = np.stack(tea_back, axis=-1)

    return svf, tcf ,tea, tea_front, tea_back
