...
def horizon(azimuth, dem, spacing):
    ...
    if azimuth == 90:
        # East
        hcos, hcos_f, hcos_b = hor2d_c(dem, spacing, fwd=True)

    elif azimuth == -90:
        # West
        hcos, hcos_f, hcos_b = hor2d_c(dem, spacing, fwd=False)

    elif azimuth == 0:
        # South
        hcos, hcos_f, hcos_b = hor2d_c(dem.transpose(), spacing, fwd=True)
        hcos, hcos_f, hcos_b = hcos.transpose(), hcos_f.transpose(), hcos_b.transpose()

    elif np.abs(azimuth) == 180:
        # South
        hcos, hcos_f, hcos_b = hor2d_c(dem.transpose(), spacing, fwd=False)
        hcos, hcos_f, hcos_b = hcos.transpose(), hcos_f.transpose(), hcos_b.transpose()

    elif azimuth >= -45 and azimuth <= 45:
        # South west through south east
        t, spacing = skew_transpose(dem, spacing, azimuth)
        h, h_f, h_b = hor2d_c(t, spacing, fwd=True)
        hcos = skew(h.transpose(), azimuth, fwd=False)
        hcos_f = skew(h_f.transpose(), azimuth, fwd=False)
        hcos_b = skew(h_b.transpose(), azimuth, fwd=False)

    elif azimuth <= -135 and azimuth > -180:
        # North west
        a = azimuth + 180
        t, spacing = skew_transpose(dem, spacing, a)
        h, h_f, h_b = hor2d_c(t, spacing, fwd=False)
        hcos = skew(h.transpose(), a, fwd=False)
        hcos_f = skew(h_f.transpose(), a, fwd=False)
        hcos_b = skew(h_b.transpose(), a, fwd=False)
        ...
        hcos_f = skew(h_f.transpose(), a, fwd=False).transpose()
        hcos_b = skew(h_b.transpose(), a, fwd=False).transpose()
...
def hor2d_c(z, spacing, fwd=True):
    ...
    topo_core.c_hor2d(z, spacing, fwd, h, h_f, h_b)
